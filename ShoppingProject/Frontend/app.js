              ...
              <div className="flex flex-col gap-2">
                {filteredItems.map(item => (
                  ...
                  <div key={item.id} className={`flex items-center justify-between p-3 rounded-xl border border-white/5 bg-black/10 ${item.isDone ? 'opacity-50' : ''}`}> 
                    <div className="flex items-center gap-3 cursor-pointer" onClick={() => toggleItem(item.id)}> {/* Når en vare klikkes, kaldes toggleItem funktionen med vare-id som argument, hvilket toggler dens status mellem købt og ikke købt. */}
